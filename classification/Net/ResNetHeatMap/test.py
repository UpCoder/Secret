...
from tools.Tools import write_result
...
def main(_):
    ...
    logits = inference(images,
                       ...
                       bottleneck=False,
                       num_blocks=[2, 2, 2, 2])
    predicted_labels = val(is_training, logits, images, labels, is_testing=True)
    predicted_paths = val_dataset.images_names
    print np.sum(predicted_labels)
    print predicted_labels
    predicted_names = [os.path.basename(path).replace('.jpg', '.tiff') for path in predicted_paths]
