...
def train(is_training, logits, images, labels, save_model_path=None):
    ...
    for x in xrange(FLAGS.max_steps + 1):
        ...
        if step % 5 == 0:
            ...
            accuracy = eval_accuracy(predictions_values, labels_values, num_category=net_config.OUTPUT_NODE)
            format_str = ('step %d, loss = %.2f, top1 error = %g, accuracy value = %g, 0 rate = %g, 1 rate = %g, 2 rate = %g  (%.1f examples/sec; %.3f '
                          'sec/batch)')

            print(format_str % (step, loss_value, top1_error_value, accuracy_value, accuracy[0], accuracy[1], accuracy[2], examples_per_sec, duration))
        if write_summary:
            summary_str = o[2]
